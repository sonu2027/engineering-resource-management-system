...
function Message() {
    ...
    useEffect(() => {
        ...
        const handleReceiveMessage = (newMsg: Message) => {
            ...
            // Update messages if chatting with sender/receiver
            if (
                selectedUser &&
                (newMsg.senderId === selectedUser._id || newMsg.receiverId === selectedUser._id)
            ) {
    ...
    const handleSelectUser = useCallback(async (otherUser: User) => {
        setSelectedUser(otherUser);
    ...
    const sendMessage = useCallback(() => {
        ...
        socket.emit("send-message", payload);
        setMessageText("");
    }, [messageText, selectedUser, user?._id, socket]);
    ...
    }, [socket, selectedImage, user?._id, selectedUser?._id, previewUrl]);
                            ...
                            {allUsers.length > 0 ? (
                                <ScrollArea className="mt-4 h-[400px]">
                                    {allUsers.map((user, index) => (
                                        <div
                                            key={user._id}
                                            onClick={() => handleSelectUser(user)}
                                            className={`cursor-pointer px-1 py-2 sm:px-4 sm:py-3 flex items-center gap-1 sm:gap-3 hover:bg-slate-100 transition-colors duration-200 ${index !== allUsers.length - 1 ? "border-b border-gray-200" : ""
                                                }`}
                                        >
                                            <div className="w-6 h-6 sm:w-10 sm:h-10 rounded-full bg-slate-300 flex items-center justify-center text-white font-bold text-xs sm:text-sm">
                                                {user.name.charAt(0).toUpperCase()}
                                ...
                                <>
                                    <p className="mb-2 text-muted-foreground">
                                        No messages yet. Start chatting with someone!
                                    </p>
                                    <Button onClick={handleShowAllUsers}>Show All Users</Button>
                                </>
                            )}
                        ...
                        <>
                            {previewUrl ? (
                                <div className="flex justify-center items-center relative">
                                    <div
                                        ...
                                        className="absolute right-4 top-4 bg-gray-700 rounded-full text-white p-2 cursor-pointer hover:bg-gray-600"
                                    >
                                        <RxCross2 className="text-2xl" />
                                    </div>
                                    <img src={previewUrl} alt="Preview" className="w-full rounded shadow" />
                            ...
                            {previewUrl ? (
                                <IoMdSend
                                    ...
                                    className="fixed bottom-8 right-10 text-green-700 text-2xl cursor-pointer hover:text-green-800"
                                />
                            ) : (
                                <div className="flex justify-center items-center">
                                    <div className="flex items-center justify-between gap-2 fixed bottom-1 w-2/3">
                                        <div className="bg-slate-950 flex justify-between items-center rounded-sm px-2">
                                            ...
                                            <FaCamera
                                                ...
                                                className="text-xl text-gray-300 cursor-pointer hover:text-gray-100"
                                            />
                                            <input
                                                type="file"
                                                accept="image/*"
